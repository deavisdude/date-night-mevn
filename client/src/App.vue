...
export default {
  ...
  async mounted() {
    const response = await axios.get('http://localhost:81/api/destinations/');
    this.items = response.data;
  },
  methods: {
    async addItem() {
      const response = await axios.post('http://localhost:81/api/destinations/', {
         name: this.name
      });
    ...
    async removeItem(item, i) {
      await axios.delete("http://localhost:81/api/destinations/" + item._id);
    ...
    async updateItem(item, i) {
      const response= await axios.put("http://localhost:81/api/destinations/" + item._id, {
