...
export default {
  ...
  methods: {
    async addItem() {
      const response = await axios.post('http://localhost:3000/api/destinations/', {
         name: this.name
      });
    ...
    async removeItem(item, i) {
      await axios.delete("http://localhost:3000/api/destinations/" + item._id);
    ...
    async updateItem(item, i) {
      const response= await axios.put("http://localhost:3000/api/destinations/" + item._id, {
